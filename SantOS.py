...
if __name__ == "__main__":
    start_bot()
# В КОНЦЕ SantOS.py добавьте:

# Экспортируем необходимые переменные для app.py
__all__ = [
    'load_data', 'save_data', 'check_bot_token', 'process_update',
    'rooms', 'user_rooms', 'user_states', 'BASE_URL', 'logger'
]

if __name__ == "__main__":
    # Если файл запускается напрямую (старый способ)
    print("⚠️ Используйте app.py для запуска бота!")
    print("Запуск через app.py обеспечивает правильную обработку ошибок.")
    
    # Все равно пытаемся запустить для обратной совместимости
    try:
        start_bot()
